...
MySQL 数据源在三阶段（本地开发、测试、生产）环境下的连接管理和行为契约。确保所有环境使用统一的 MySQL 8.4 LTS 数据库，消除 H2 与 MySQL 的方言差异风险。
...
The Actuator health endpoint SHALL report MySQL DataSource connectivity.
